...
import {
  ...
  ObjectTasks,
  ObjectTransactions,
  WebCalendarDisableiCalendar,
  WebCalendarEnableiCalendar,
  WebCalendarGenerateNewiCalendarURL,
  WebCalendariCalendarURLTooltip,
  WebCalendarICSImport,
  WebCalendarICSImported,
  WebCalendarImportiCalendarFeed,
  WebCalendarOtherCalendars,
  WebCalendarOtherCalendarsAstronomy,
  WebCalendarShowCalendars,
  WebGlobalActionExportCalendar,
  WebGlobalActionImportCalendars,
  WebGlobalBudgetMonth,
  WebGlobalDisplay,
  WebGlobalDisplayDay,
  WebGlobalDisplayWeek,
  WebGlobalName,
...
const state: {
  ...
  from: CivilDate.now().toYearMonth(),
  next: CivilDate.now(),
  picker: Stream(CivilDate.now().toYearMonth()),
...
export function Calendar(): m.Component {
  ...
  function setDateRange(from: Timestamp): void {
    switch (state.display()) {
      case "day":
        ...
        dateRange.from = from.toCivilDate();
        dateRange.to = CivilDate.fromString(dateRange.from.toJSON());
        state.dateRange = dateRange;

        const day = Timestamp.fromString(from.toString());
        day.addDays(1);
        ...
        break;
      case "month":
        state.dateRange = from.toCivilDateRangeCalendar(
          AuthAccountState.data().preferences.formatWeek8601,
        );
        from.addDays(15);

        const month = Timestamp.fromString(from.toString());
        month.addMonths(1);
        ...
        break;
      default:
        state.dateRange = from.toCivilDateRangeWeek(
          AuthAccountState.data().preferences.formatWeek8601,
        );

        state.next = Timestamp.fromCivilDate(
          state.dateRange.from,
        ).toCivilDateWeekFrom(false);
        state.previous = Timestamp.fromCivilDate(
          state.dateRange.to,
  ...
  function newCalendarEvent(): CalendarEvent {
    ...
    if (state.dateRange.from.toJSON() === state.dateRange.to.toJSON()) {
      ...
      event.dateStart = Timestamp.now().toCivilDate().toJSON();
    }

    const time = Timestamp.now();
    time.addMinutes(-1 * time.timestamp.getMinutes() + 60);
    event.timeStart = time.toCivilTime().toString(true);

    return event;
  }

  const eventRanges = Stream.lift(
    (
      _calendarEvents,
      _cookMealPlans,
      _healthLogs,
      ...
      const dateRange = Timestamp.fromCivilDate(
        CivilDate.fromString(`${picker.getYear()}-${picker.getMonth()}-01`),
      ).toCivilDateRangeCalendar(
        AuthAccountState.data().preferences.formatWeek8601,
      );
  ...
  return {
    oninit: async (): Promise<void> => {
      ...
      AppState.setLayoutApp({
        ...
        toolbarActionButtons: [
            ...
            ...{
              onclick: (): void => {
                AppState.setLayoutAppForm(FormOverlayCalendarEvent, {
                  ...
                  ...{
                    dateStart:
                      state.dateRange.from.toJSON() ===
                      state.dateRange.to.toJSON()
                        ? state.dateRange.to.toJSON()
                        : CivilDate.now().toJSON(),
                  },
                });
              },
            },
          },
          {
            ...
            onclick: (): void => {
              state.formShopItemIngredients = CookMealPlanState.data().filter(
                (plan) => {
                  const date = CivilDate.fromString(plan.date as string);
                  return (
                    date <= state.dateRange.to && date >= state.dateRange.from
                  );
                },
              );
            ...
            permitted:
              ...
              !GlobalState.hideComponentIncludes("Shop"),
            requireOnline: true,
          },
          {
                ...
                AppState.setLayoutAppForm(FormOverlayPlanTask, {
                  ...
                  ...{
                    dueDate:
                      state.dateRange.from.toJSON() ===
                      state.dateRange.to.toJSON()
                        ? Timestamp.fromCivilDate(state.dateRange.to).toString()
                        : [],
                  },
                });
              },
      ...
      if (
        m.route.param().display === undefined &&
        AppState.getSessionDisplay() === DisplayEnum.XSmall
      ) {
        state.display("day");
      }
      ...
      if (m.route.param().from !== undefined || m.route.param().from === "") {
        date = Timestamp.fromCivilDate(
          CivilDate.fromString(m.route.param().from),
        );
      }

      state.picker(date.toCivilDate().toYearMonth());
      setDateRange(date);
    ...
    view: (): m.Children => {
      const loaded =
        BudgetRecurrenceState.isLoaded() &&
        CalendarEventState.isLoaded() &&
        CookMealPlanState.isLoaded() &&
        PlanTaskState.isLoaded();
        ...
        m("div.Calendar__picker", [
          m("div.Calendar__title-container#picker-title", [
            m(Icon, {
              icon: Icons.Previous,
              onclick: async () => {
                state.picker(state.picker().addMonths(-1));
              },
            }),
          ...
          m("div.Calendar__calendar.Calendar__calendar--picker#picker", [
            Weekday.getValues(
              ...
              return m(
                "span.Calendar__date.Calendar__date--day",
                {
                  key: day,
                },
                m("span", day[0]),
            ...
            Timestamp.getCivilDatesFromDateRange(
              Timestamp.fromCivilDate(
                CivilDate.fromString(
                  `${state.picker().getYear()}-${state.picker().getMonth()}-01`,
                ),
              ).toCivilDateRangeCalendar(
                AuthAccountState.data().preferences.formatWeek8601,
              ),
                ...
                m.route.Link,
                {
                  class: SetClass({
                    ...
                    "Calendar__date--active":
                      (date.valueOf() === state.dateRange.from.valueOf() &&
                        date.valueOf() === state.dateRange.to.valueOf()) ||
                      (date.valueOf() >= state.dateRange.from.valueOf() &&
                        date.valueOf() <= state.dateRange.to.valueOf()),
                    "Calendar__date--active-end":
                      date.valueOf() === state.dateRange.to.valueOf(),
                    "Calendar__date--active-start":
                      date.valueOf() === state.dateRange.from.valueOf(),
                    "Calendar__date--outside":
                      date.getMonth() !== state.picker().getMonth(),
                    "Calendar__date--today":
                      date.valueOf() === state.today.valueOf(),
                  }),
                      ...
                      return range[date.toJSON()] === undefined
                        ...
                        : m(
                            "span",
                            {
                              style: {
                                color: Color.toHex(
                                  Colors.calendarEvent(
                                    range[date.toJSON()][0].color,
                                  ),
                                ),
                              },
                            },
            ...
            filters: [
              {
                ...
                onclick: (e): void => {
                  m.route.set(
                    ...
                    {
                      display: e === "today" ? state.display() : e,
                      from: e === "today" ? state.today : m.route.param().from,
                    },
                    {
                      state: {
                        key: Date.now(),
                ...
                selected: (): string[] => {
                  return [
                    state.today.valueOf() >= state.dateRange.from.valueOf() &&
                    state.today.valueOf() <= state.dateRange.to.valueOf()
                      ? "today"
                      : "",
                    state.display(),
                  ];
              ...
              },
              {
                ...
                onclick: async (e): Promise<void | Err> => {
                  if (e.startsWith("health")) {
                    await AuthAccountState.hideCalendarHealthLogs(
                      AuthHouseholdState.findMember(e.replace("health_", ""))
                        .id,
                    );
                  } else {
                    switch (e) {
                ...
                selected: (): string[] => {
                  return [
                    AuthAccountState.data().preferences.hideCalendarEvents
                      ? ""
                      : "events",
                    ...AuthHouseholdState.members().map((member) => {
                      if (
                ...
                value: [
                  ...
                  ...(GlobalState.hideComponentIncludes("Health")
                    ? []
                    : AuthHouseholdState.findMemberNames(null, true).map(
                        (name) => {
                    ...
                    : [
                        {
                          color: Colors.planTask(
                            AuthHouseholdState.findID(
                              AuthAccountState.data().primaryAuthHouseholdID,
                            ).preferences.colorPlanTaskEvents,
                            "",
                          ),
                          icon: Icons.PlanTask,
                          id: "tasks",
                  ...
                  ...(GlobalState.hideComponentIncludes("Budget")
                    ...
                    : [
                        {
                          color: Colors.budgetRecurrrence(
                            AuthHouseholdState.findID(
                              AuthAccountState.data().primaryAuthHouseholdID,
                            ).preferences.colorBudgetRecurrenceEvents,
                          ),
              ...
              },
              {
                ...
                onclick: async (e): Promise<void | Err> => {
                  switch (e) {
                    ...
                    default:
                      const c = CalendarICalendarState.data().find(
                        (icalendar) => {
                      ...
                      if (c !== undefined && c.id !== null) {
                        return AuthAccountState.update({
                          ...AuthAccountState.data(),
                          ...{
                            hideCalendarICalendars: PushPopStringArray(
                              c.id,
                              AuthAccountState.data().hideCalendarICalendars,
                            ),
                          },
                        });
                      }
                ...
                selected: (): string[] => {
                  return [
                    ...
                    ...CalendarICalendarState.data()
                      .filter((icalendar) => {
                        return !AuthAccountState.data().hideCalendarICalendars.includes(
                          `${icalendar.id}`,
                        );
                      })
                      .map((icalendar) => {
            ...
            ? m(
                ...
                [
                  ...
                  m(Button, {
                    ...
                    onclick: async (): Promise<void> => {
                      return AuthAccountState.updateICalendarID().then(() => {
                        m.redraw();
                      });
                    },
                          ...
                          buttons: [
                            {
                              ...
                              onclick: async (): Promise<void> => {
                                return CalendarICalendarState.delete(
                                  state.formCalendarICalendar.data.id,
                                ).then(() => {
                                  state.formCalendarICalendar.visible = false;
                                });
                              },
                              permitted:
                                state.formCalendarICalendar.data.id !== null &&
                                GlobalState.permitted(
                                  PermissionComponentsEnum.Calendar,
                                  true,
                                  state.formCalendarICalendar.data
                                    .authHouseholdID,
                                ),
                              requireOnline: false,
                            },
                            ...
                            },
                            {
                              ...
                              onclick: async (): Promise<void> => {
                                if (
                                  ...
                                  return CalendarICalendarState.create(
                                    state.formCalendarICalendar.data,
                                  ).then(() => {
                                    state.formCalendarICalendar.visible = false;
                                  });
                                ...
                                return CalendarICalendarState.update(
                                  state.formCalendarICalendar.data,
                                ).then(() => {
                                  state.formCalendarICalendar.visible = false;
                                });
                          ...
                          title: {
                            name: `${
                              state.formCalendarICalendar.data.id === null
                                ...
                                : AuthAccountState.translate(ActionUpdate)
                            } ${AuthAccountState.translate(ObjectiCalendarFeed)}`,
                          },
                        },
                        [
                          ...
                          m(FormItem, {
                            ...
                            name: AuthAccountState.translate(WebGlobalName),
                            tooltip:
                              AuthAccountState.translate(WebGlobalNameTooltip),
                          }),
          ...
          m("div.Calendar__calendar#calendar", [
            state.display() === "day"
              ...
              : Weekday.getValues(
                  ...
                  return m(
                    "div.CalendarDay__container.CalendarDay__container--day",
                    m("p", weekday),
                  );
                }),
            Timestamp.getCivilDatesFromDateRange(state.dateRange).map(
              (date: CivilDate, i) => {
                return m(CalendarDay, {
                  ...
                  dropdownItems: [
                    ...
                    },
                    {
                      ...
                      onclick: (): void => {
                        AppState.setLayoutAppForm(FormOverlayPlanTask, {
                          ...
                          ...{
                            authAccountID: AuthAccountState.data().id,
                            dueDate: Timestamp.fromCivilDate(date).toString(),
                          },
                        });
                      },
                      permitted: GlobalState.permitted(
                    ...
                    },
                    {
                      ...
                      onclick: (): void => {
                        setFormShopItemIngredients(
                          CookMealPlanState.data().filter((plan) => {
                      ...
                      permitted:
                        ...
                        eventRanges()[2][date.toJSON()] !== undefined &&
                        eventRanges()[2][date.toJSON()].length > 0,
                      requireOnline: true,
                    },
                  ],
                  events: loaded
