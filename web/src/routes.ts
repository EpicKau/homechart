...
export default {
      ...
      setTimeout(async () => {
        return IndexedDB.clear()
          .then(async () => {
            ...
            return API.read(
              `/api/v1/auth/demo?code=${AuthAccountState.data().iso639Code}&timezone=${
  ...
  "/oidc": {
    onmatch: async (args: RouteLinkAttrs): Promise<void> => {
      return OIDCState.readResponse(args.code, args.provider, args.state).catch(
        async () => {
          return GlobalState.signOut();
        },
      );
  ...
  "/settings/notifications": BuildRoute(async () => {
    return (await import("./views/SettingsNotifications"))
