  ...
  return {
    view: (vnode): m.Children => {
      ...
      const color =
        vnode.attrs.event.color === ""
          ? vnode.attrs.event.participants.length > 0 &&
            AuthHouseholdState.findMember(vnode.attrs.event.participants[0])
              .color === ""
      ...
      return vnode.attrs.loaded
              ...
              onclick: (e: m.Event<MouseEvent>) => {
                ...
                if (vnode.attrs.event.budgetRecurrence !== undefined) {
                  AppState.setLayoutAppForm(
                    FormOverlayBudgetTransaction,
                    vnode.attrs.event.budgetRecurrence,
                  );
                  return;
                }
                if (vnode.attrs.event.cookMealPlans !== undefined) {
                  ...
                  vnode.attrs.event.healthLogOutputs !== undefined
                ) {
                ...
                AppState.setLayoutAppForm(
                  FormOverlayCalendarEvent,
                  vnode.attrs.event,
                );
              },
              style: {
                background:
                  Timestamp.now() < end && Timestamp.now() > start
                ...
                filter:
                  (vnode.attrs.event.duration !== 0 && Timestamp.now() > end) ||
                  CivilDate.now() >
                    Timestamp.fromString(vnode.attrs.event.timestampStart!) // eslint-disable-line @typescript-eslint/no-non-null-assertion
                      .toCivilDate()
                    ? "var(--filter_light)"
                    : undefined,
              },
            ...
            [
              m("span.CalendarDayEvent__event", [
                m("div.CalendarDayEvent__title", [
                  m(Icon, {
                    icon:
                      vnode.attrs.event.icon === undefined
                        ? Icons.Calendar
                ...
                vnode.attrs.event.duration !== 0 &&
                vnode.attrs.event.duration % (60 * 24) !== 0 &&
                (vnode.attrs.date.toJSON() ===
                  Timestamp.fromString(vnode.attrs.event.timestampStart!) // eslint-disable-line @typescript-eslint/no-non-null-assertion

                    .toCivilDate()
                    .toJSON() ||
                  vnode.attrs.date.toJSON() === end.toCivilDate().toJSON())
                  ? m(
                      "span",
                      `${
                        vnode.attrs.date.toJSON() ===
                        Timestamp.fromString(vnode.attrs.event.timestampStart!) // eslint-disable-line @typescript-eslint/no-non-null-assertion
                          ...
                          : CivilTime.fromString("00:00").toString(
                ...
                ? m("div.CalendarDayEvent__description", [
                    vnode.attrs.event.participants.length === 0
                      ...
                      : vnode.attrs.event.participants.map((participant) => {
                          const p =
                            AuthHouseholdState.findMemberName(participant);
                          ...
                          return p === ""
                            ...
                            : m("p.CalendarDayEvent__participant", [
                                ...
                                m(
                                  "span",
                                  AuthHouseholdState.findMemberName(
                                    participant,
                                  ),
                                ),
                              ]);
                    ...
                    vnode.attrs.event.travelTime > 0
                      ? m("p", [
                          ...
                          m(
                            "span",
                            `Leave by ${Timestamp.fromString(
                              vnode.attrs.event.timestampStart!,
                    ...
                    vnode.attrs.event.location === ""
                      ? []
                      : m(
                          "a.GlobalLink.CalendarDayEvent__location",
                          {
                      ...
                      : m(
                          ...
                          [
                            ...
                            m(
                              ...
                              {
                                class:
                                  vnode.attrs.event.id !== null && expand
                    ...
                    vnode.attrs.event.cookMealPlans === undefined
                      ...
                      : vnode.attrs.event.cookMealPlans.map((plan) => {
                          const recipe = CookRecipeState.findID(
                            plan.cookRecipeID,
                          );

                          return m(
                            "div.CalendarDayEvent__extra",
                            {
                              onclick: () => {
                                AppState.setLayoutAppForm(
                                  FormOverlayCookMealPlan,
                                  plan,
                                );
                              },
                            },
                    ...
                    vnode.attrs.event.healthLogInputs !== undefined &&
                    vnode.attrs.event.healthLogInputs.length > 0
                      ? m("div.CalendarDayEvent__extra", [
                          m(
                            "div.CalendarDayEvent__extra--container",
                            vnode.attrs.event.healthLogInputs.map((log) => {
                              const item = HealthItemState.findID(
                                log.healthItemID,
                              );
                      ...
                      : [],
                    vnode.attrs.event.healthLogOutputs !== undefined &&
                    vnode.attrs.event.healthLogOutputs.length > 0
                      ? m("div.CalendarDayEvent__extra", [
                          m(
                            "div.CalendarDayEvent__extra--container",
                            vnode.attrs.event.healthLogOutputs.map((log) => {
                              const item = HealthItemState.findID(
                                log.healthItemID,
                              );
        ...
        : m(
            `div.CalendarDayEvent__item.CalendarDayEvent__item--loading.${Animate.class(Animation.Pulse)}`,
          );
    },
  };
}
